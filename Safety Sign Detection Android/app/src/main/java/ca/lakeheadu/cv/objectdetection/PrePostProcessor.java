...
class Result {
    ...
    public String toString() {
        return "Result [classIndex=" + classIndex + ", score=" + score + "]";
    }
};
...
public class PrePostProcessor {
    ...
    // model output is of size 25200*8
    private static int mOutputRow = 25200; // as decided by the YOLOv5 model for input image of size 640*640
    private static int mOutputColumn = 8; // left, top, right, bottom, score and 3 class probability
    static float mThreshold = 0.50f; // score above which a detection is generated
    ...
    static ArrayList<Result> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        ...
        for (int i = 0; i < mOutputRow; i++) {
            if (outputs[i * mOutputColumn + 4] > mThreshold) {
                ...
                Result result = new Result(cls, outputs[i * 8 + 4], rect);
